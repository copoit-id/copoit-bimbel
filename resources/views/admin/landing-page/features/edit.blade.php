...
<div class="mb-6">
    <div class="flex items-center space-x-2 text-sm text-gray-600 mb-2">
        ...
        <span>Edit Keunggulan</span>
    </div>
    <h1 class="text-2xl font-bold text-gray-900">Edit Keunggulan</h1>
</div>

<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    <form action="{{ route('admin.landing-page.features.update', $feature) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $feature->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="mt-2 p-2 border rounded bg-gray-50 flex items-center gap-2">
                        ...
                        <i id="icon-preview" class="{{ old('icon', $feature->icon) ?: 'ri-star-line' }} text-2xl text-primary"></i>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', $feature->order) }}" min="0"
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $feature->is_active) ? 'checked' : '' }}
