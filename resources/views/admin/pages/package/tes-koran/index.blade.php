...
<div class="flex justify-between items-center">
    <x-breadcrumb>
        <x-slot name="items">
            ...
            <x-breadcrumb-item href="" title="Tes Koran" />
        </x-slot>
    </x-breadcrumb>
</div>

<div class="package-bimbel bg-white p-8 rounded-lg border border-border">
    <x-page-desc title="Tes Koran - {{ $package->name }}" description="Pilih tes koran yang akan ditambahkan ke paket">
    </x-page-desc>
    ...
    <div class="relative overflow-x-auto mt-4">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
                @forelse($tesKorans as $tesKoran)
                @php
                $isLinked = $tesKoran->detailPackages->where('package_id', $package->package_id)->isNotEmpty();
                @endphp
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">
                                    Kolom: {{ $tesKoran->columns_count }} | Baris: {{ $tesKoran->rows_count }}
                                </p>
                            </div>
                        </div>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                        <div class="flex flex-col items-center">
                            ...
                            <p>Belum ada tes koran tersedia</p>
                            <a href="{{ route('admin.tes-koran.create') }}" class="text-primary hover:underline mt-2">
                                Buat tes koran baru
                            </a>
                        </div>
                    </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showNotification(message, type) {
        ...
        notification.className = `fixed bottom-4 right-4 px-4 py-3 rounded-lg z-50 ${type === 'success' ? 'bg-green-100 border border-green-400 text-green-700' : 'bg-red-100 border border-red-400 text-red-700'}`;
