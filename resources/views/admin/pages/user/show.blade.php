...
<div class="space-y-6" x-data="{ activeTab: 'profile' }">
    ...
    <section class="overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-primary to-emerald-700 text-white shadow-sm">
        <div class="flex flex-col gap-5 p-6 md:flex-row md:items-center md:p-8">
            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=ffffff&color=166534&size=160" alt="{{ $user->name }}" class="h-20 w-20 rounded-2xl border-4 border-white/30 shadow-lg">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-2 shadow-sm">
        <div class="flex gap-1 overflow-x-auto" role="tablist" aria-label="Kategori detail user">
            @foreach ([
                ...
                <button type="button" @click="activeTab = '{{ $key }}'" :class="activeTab === '{{ $key }}' ? 'bg-primary text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'" class="inline-flex shrink-0 items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium transition">
    ...
    <div x-show="activeTab === 'program'" x-cloak class="space-y-6">
        <div class="grid gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-5"><p class="text-sm text-emerald-700">Akses paket aktif</p><p class="mt-1 text-3xl font-bold text-emerald-800">{{ $activePackageAccess->count() }}</p></div>
            <div class="rounded-xl border border-blue-100 bg-blue-50 p-5"><p class="text-sm text-blue-700">Total riwayat paket</p><p class="mt-1 text-3xl font-bold text-blue-800">{{ $user->userPackageAccess->count() }}</p></div>
            <div class="rounded-xl border border-violet-100 bg-violet-50 p-5"><p class="text-sm text-violet-700">Akses kelas langsung</p><p class="mt-1 text-3xl font-bold text-violet-800">{{ $user->classAccess->count() }}</p></div>
        </div>
    ...
    <div x-show="activeTab === 'payment'" x-cloak class="space-y-6">
        <div class="grid gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-5"><p class="text-sm text-emerald-700">Pembayaran paket diterima</p><p class="mt-1 text-2xl font-bold text-emerald-800">{{ $formatMoney($paymentSummary['paid']) }}</p></div>
            <div class="rounded-xl border border-amber-100 bg-amber-50 p-5"><p class="text-sm text-amber-700">Tagihan rutin tertunggak</p><p class="mt-1 text-2xl font-bold text-amber-800">{{ $formatMoney($paymentSummary['outstanding']) }}</p></div>
            <div class="rounded-xl border border-blue-100 bg-blue-50 p-5"><p class="text-sm text-blue-700">Total dokumen transaksi</p><p class="mt-1 text-3xl font-bold text-blue-800">{{ $user->payments->count() + $user->billInvoices->count() }}</p></div>
        </div>
        <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    ...
    <div x-show="activeTab === 'attendance'" x-cloak class="space-y-6">
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <div class="rounded-xl border border-blue-100 bg-blue-50 p-5"><p class="text-sm text-blue-700">Total sesi tercatat</p><p class="mt-1 text-3xl font-bold text-blue-800">{{ $attendanceSummary['total'] }}</p></div>
            <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-5"><p class="text-sm text-emerald-700">Hadir</p><p class="mt-1 text-3xl font-bold text-emerald-800">{{ $attendanceSummary['present'] }}</p></div>
            <div class="rounded-xl border border-amber-100 bg-amber-50 p-5"><p class="text-sm text-amber-700">Terlambat</p><p class="mt-1 text-3xl font-bold text-amber-800">{{ $attendanceSummary['late'] }}</p></div>
            <div class="rounded-xl border border-red-100 bg-red-50 p-5"><p class="text-sm text-red-700">Tidak hadir</p><p class="mt-1 text-3xl font-bold text-red-800">{{ $attendanceSummary['absent'] }}</p></div>
            <div class="rounded-xl border border-violet-100 bg-violet-50 p-5"><p class="text-sm text-violet-700">Persentase hadir</p><p class="mt-1 text-3xl font-bold text-violet-800">{{ $attendanceSummary['rate'] !== null ? $attendanceSummary['rate'].'%' : '—' }}</p></div>
        </div>
        <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
