...
@section('title', __('Akses User'))
@section('content')

<div class="flex justify-between items-center">
    <x-breadcrumb>
        <x-slot name="items">
            <x-breadcrumb-item href="" title="{{ __('Akses User') }}" />
        </x-slot>
    </x-breadcrumb>
</div>
<x-page-desc title="{{ __('Akses User - Kelola & Pengajuan') }}"></x-page-desc>
...
<div class="grid grid-cols-3 gap-4 mt-6">
    <div class="bg-primary/5 border border-primary/50 rounded-lg p-4">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-primary">{{ __('Total User Akses') }}</p>
    ...
    <div class="bg-primary/5 border border-primary/50 rounded-lg p-4">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-primary">{{ __('Akses Aktif') }}</p>
    ...
    <div class="bg-primary/5 border border-primary/50 rounded-lg p-4">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-primary">{{ __('Pengajuan Pending') }}</p>
...
{{-- <div class="flex items-center gap-3 mt-8">
    ...
    <button type="button"
        ...
        {{ __('Pengajuan Akses (:count)', ['count' => $pendingRequestCount]) }}
    </button>
</div> --}}
...
<div id="akses-section-packages" data-layout="grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div class="flex items-center justify-between text-sm text-gray-500 mb-4">
            <span>{{ __('Total User: :count', ['count' => $package->user_access_count ?? 0]) }}</span>
            <span>{{ __('Aktif: :count', ['count' => $package->active_users_count ?? 0]) }}</span>
        </div>
...
<div id="akses-section-requests" data-layout="block" class="hidden mt-6">
    <div class="bg-white rounded-lg border border-gray-200">
        <div class="p-6 border-b border-gray-100">
            <h3 class="text-lg font-semibold text-gray-900">{{ __('Pengajuan Akses Gratis Bersyarat') }}</h3>
            <p class="text-sm text-gray-500">{{ __('Review bukti peserta sebelum memberikan akses paket gratis bersyarat.') }}</p>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm text-gray-600">
                <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Pengguna') }}</th>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Paket') }}</th>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Bukti & Catatan') }}</th>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Status') }}</th>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Diajukan') }}</th>
                        <th class="px-6 py-3 text-left font-semibold">{{ __('Aksi') }}</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                @else
                                <span class="text-xs text-gray-400">{{ __('Belum ada bukti') }}</span>
                                @endif
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-2">
                                <form action="{{ route('admin.akses.requests.approve', $requestItem->user_package_access_id) }}"
                                    method="POST" onsubmit="return confirm(@json(__('Setujui pengajuan akses ini?')));">
                                    @csrf
                                    <button type="submit"
                                        ...
                                        <i class="ri-check-line"></i> {{ __('Setujui') }}
                                    </button>
                                </form>
                                <form action="{{ route('admin.akses.requests.reject', $requestItem->user_package_access_id) }}"
                                    method="POST" onsubmit="return confirm(@json(__('Tolak pengajuan akses ini?')));">
                                    @csrf
                                    <button type="submit"
                                        ...
                                        <i class="ri-close-line"></i> {{ __('Tolak') }}
                                    </button>
                                </form>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                            {{ __('Tidak ada pengajuan akses menunggu persetujuan.') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<script>
    ...
    console.log(@json(__('Package access management loaded')));
});
</script>
