                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="flex items-center mb-1">
                            <input type="checkbox" name="is_for_sale" value="1" {{ old('is_for_sale', $material->is_for_sale) ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
                        ...
                        <select name="category_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
                            ...
                            @foreach($categories as $category)
                                ...
                                <optgroup label="{{ $category->name }}">
                                    <option value="{{ $category->category_id }}" {{ old('category_id', $selectedCategory) == $category->category_id ? 'selected' : '' }}>Semua {{ $category->name }}</option>
                                    @foreach($category->activeChildren as $child)
                                    <option value="{{ $child->category_id }}" {{ old('category_id', $selectedCategory) == $child->category_id ? 'selected' : '' }}>{{ $child->name }}</option>
                                    @endforeach
                                </optgroup>
                                @endif
                            @endforeach
