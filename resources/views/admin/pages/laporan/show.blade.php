        ...
        <div class="relative overflow-x-auto mt-5">
            <table class="w-full text-sm text-left text-gray-600">
                ...
                <tbody>
                    @forelse(($liveScore['rows'] ?? []) as $row)
                        <tr class="border-b border-gray-100">
                            ...
                            @foreach ($liveScore['subtests'] ?? [] as $subtest)
                                ...
                                @endphp
                                <td
                                    class="px-4 py-3 text-center font-semibold {{ is_null($scoreValue) ? 'text-gray-400' : 'text-gray-800' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 3 + count($liveScore['subtests'] ?? []) }}"
                ...
                <div class="rounded-2xl border border-border bg-white transition data-[hidden=true]:hidden"
                    ...
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between px-5 py-5">
                        <div class="flex items-center gap-3">
                            <div class="relative">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name ?? 'User') }}&background=444444&color=fff"
                                    class="w-14 h-14 rounded-full ring-2 ring-primary/10"
                                    alt="{{ $user->name ?? 'User' }}">
                                <span
                                    class="absolute -bottom-1 -right-1 bg-primary text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <button type="button" data-participant-toggle
                                data-target="participant-attempts-{{ $index }}"
                    ...
                    <div id="participant-attempts-{{ $index }}"
                        class="hidden border-t border-border/50 bg-gradient-to-b from-gray-50 to-white rounded-b-2xl">
                                    ...
                                    <tbody>
                                        @foreach ($participant['attempts'] as $attempt)
                                            @php
                                                ...
                                                $attemptFinished = $attempt->finished_at
                                                    ? Carbon::parse($attempt->finished_at)
                                                    : null;
                                                ...
                                                $durationLabel =
                                                    $attemptStarted && $attemptFinished
                                                        ? $attemptStarted->diffForHumans($attemptFinished, true)
                                                        : '-';
                                            @endphp
                                            <tr class="border-t border-gray-100 hover:bg-gray-50/80 transition">
                                                <td class="px-4 py-3">
                                                    <span
                                                        class="inline-flex items-center gap-2 text-xs font-mono bg-gray-100 px-3 py-1 rounded-full text-gray-600">
                                                        <i class="ri-hashtag"></i>{{ $attempt->attempt_token }}
                                                    </span>
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <p class="text-lg font-semibold text-gray-900">
                                                        {{ round($attempt->total_score ?? 0, 1) }}</p>
                                                    <p class="text-[11px] text-gray-500">{{ $questions }} soal</p>
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <div
                                                        class="inline-flex items-center justify-center gap-3 text-xs font-semibold">
                                                        <span class="flex items-center gap-1 text-green-900"
                                                            title="Benar">
                                                            <i
                                                                class="ri-check-line text-base w-[25px] h-[25px] bg-green-900 flex justify-center items-center rounded-full text-white"></i>
                                                            {{ $attempt->total_correct ?? 0 }}
                                                        </span>
                                                        <span class="flex items-center gap-1 text-red-900" title="Salah">
                                                            <i
                                                                class="ri-close-line text-base w-[25px] h-[25px] bg-red-900 flex justify-center items-center rounded-full text-white"></i>
                                                            {{ $attempt->total_wrong ?? 0 }}
                                                        </span>
                                                        <span class="flex items-center gap-1 text-gray-500"
                                                            title="Kosong">
                                                            <i
                                                                class="ri-subtract-line text-base w-[25px] h-[25px] bg-gray-500 flex justify-center items-center rounded-full text-white"></i>
                                                            {{ $attempt->total_unanswered ?? 0 }}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-4 py-3 text-center">
                                                    <p class="font-medium text-gray-800">
                                                        {{ $attemptStarted ? $attemptStarted->translatedFormat('d M Y') : '-' }}
                                                    </p>
                                                    <p class="text-xs text-gray-500">
                                                        {{ $attemptStarted ? $attemptStarted->format('H:i') : '' }}</p>
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <p class="font-medium text-gray-800">
                                                        {{ $attemptFinished ? $attemptFinished->translatedFormat('d M Y') : '-' }}
                                                    </p>
                                                    <p class="text-xs text-gray-500">
                                                        {{ $attemptFinished ? $attemptFinished->format('H:i') : '' }}</p>
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <div class="inline-flex flex-wrap items-center justify-center gap-2">
                                                        ...
                                                        @if (!in_array($attempt->attempt_status, ['completed', 'pending_release']))
                                                            <button type="button"
                                                                data-open-time-modal="time-modal-{{ $user->id }}"
                                                                class="inline-flex items-center gap-2 px-4 py-1.5 text-xs font-semibold rounded-full border border-primary text-primary hover:bg-primary hover:text-white transition">
                                                        ...
                                                        @endif
                                                        <form
                                                            action="{{ route('admin.laporan.reset-attempt', [$tryout->tryout_id, $attempt->attempt_token]) }}"
                                                            method="POST"
                    ...
                    <div class="relative bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none"
                                data-close-time-modal>&times;</button>
                        </div>
                        <form action="{{ route('admin.laporan.add-time', [$tryout->tryout_id, $user->id]) }}"
                            method="POST">
                            @csrf
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('[data-participant-toggle]').forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    if (label) {
                        label.textContent = target.classList.contains('hidden') ? 'Lihat Attempt' :
                            'Sembunyikan';
                    }
                });
            });
