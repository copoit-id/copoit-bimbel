                    ...
                    <div class="mt-6 flex h-44 items-end gap-3 border-b border-slate-100 px-1 pb-7 sm:gap-5">
                        @foreach($scoreTrend as $result)
                            <div class="group flex min-w-0 flex-1 flex-col items-center justify-end gap-2">
                                <span class="text-xs font-bold text-gray-700">{{ number_format((float) $result->score, 0) }}</span>
                                <div class="w-full max-w-10 rounded-t-md bg-primary/85 transition-colors group-hover:bg-primary" style="height: {{ max(10, min(100, ((float) $result->score / $scoreTrendMaximum) * 100)) }}%"></div>
                                <span class="w-full truncate text-center text-[11px] text-gray-400" title="{{ $result->tryout?->name ?? 'Ujian' }}">{{ $result->finished_at?->format('d M') }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
