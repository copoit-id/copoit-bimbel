...
<body class="min-h-screen bg-slate-50 text-gray-800" data-app-selects>
        ...
        <div class="responsive-shell mx-auto flex max-w-[1440px] items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 sm:px-5">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                @if($children->isNotEmpty())
                    <form method="POST" action="{{ route('parent.select-child') }}">
                        @csrf
                        <label class="sr-only" for="parent-child-selector">Pilih anak</label>
                        <select id="parent-child-selector" name="child_id" onchange="this.form.submit()" class="max-w-36 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-gray-700 focus:border-primary focus:ring-primary sm:max-w-xs">
                            @foreach($children as $listedChild)
                                <option value="{{ $listedChild->id }}" @selected((int) ($child?->id) === (int) $listedChild->id)>{{ $listedChild->name }}</option>
                            @endforeach
                        </select>
                    </form>
                @endif
    ...
    <div class="responsive-shell mx-auto grid max-w-[1440px] gap-4 px-3 pb-6 pt-4 sm:px-4 lg:grid-cols-[248px_minmax(0,1fr)] lg:gap-6">
        <aside class="h-fit rounded-xl border border-slate-200 bg-white p-3 lg:sticky lg:top-24">
            <a href="{{ route('parent.dashboard') }}" class="mb-4 flex items-center gap-3 rounded-lg border border-slate-100 bg-slate-50 px-3 py-3">
            ...
            <nav class="flex gap-1 overflow-x-auto pb-1 text-sm font-semibold lg:block lg:space-y-1 lg:overflow-visible" aria-label="Navigasi Orang Tua">
                @foreach($parentNavigationItems as $item)
                    <a href="{{ route($item['route']) }}" class="flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 transition-colors {{ $item['is_active'] ? 'bg-primary text-white' : 'text-gray-600 hover:bg-primary/5 hover:text-primary' }}"><i class="{{ $item['icon'] }} text-lg"></i><span>{{ $item['label'] }}</span></a>
                @endforeach
            </nav>
        </aside>
