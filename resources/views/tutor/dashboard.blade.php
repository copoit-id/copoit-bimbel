...
<div class="space-y-6">
    <section class="rounded-2xl bg-gradient-to-br from-primary to-primary/80 p-6 text-white shadow-sm">
        ...
        <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a href="{{ route('tutor.schedule.index') }}#jadwal-mingguan" class="inline-flex w-fit items-center gap-2 rounded-lg bg-white/15 px-4 py-2 text-sm font-semibold text-white hover:bg-white/25"><i class="ri-calendar-line"></i>Lihat jadwal</a>
        </div>
    </section>
    ...
    <section class="grid gap-6 xl:grid-cols-3">
        <article class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm xl:col-span-2">
            <div class="flex items-center justify-between gap-3"><div><h2 class="font-bold text-gray-900">Jadwal terdekat</h2><p class="mt-1 text-sm text-gray-500">Sesi berikutnya yang perlu Anda siapkan.</p></div><a href="#jadwal-mingguan" class="text-sm font-semibold text-primary hover:underline">Jadwal mingguan</a></div>
    ...
    <section id="jadwal-mingguan" class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div><h2 class="font-bold text-gray-900">Jadwal Mingguan</h2><p class="mt-1 text-sm text-gray-500">{{ $weekDates->first()->locale('id')->translatedFormat('d M') }} – {{ $weekDates->last()->locale('id')->translatedFormat('d M Y') }}</p></div><div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-7">@foreach($weekDates as $dayNumber => $date)<article class="flex min-h-[190px] flex-col overflow-hidden rounded-lg border border-gray-200"><header class="border-b border-gray-200 bg-gray-50 px-3 py-2.5"><p class="text-sm font-bold text-gray-800">{{ $dayLabels[$dayNumber] }}</p><p class="text-xs text-gray-500">{{ $date->locale('id')->translatedFormat('d M') }}</p></header><div class="flex-1 space-y-2 p-2">@forelse($weeklySessions->get($dayNumber, collect()) as $session)<div class="rounded-lg border border-gray-100 bg-slate-50 p-3"><p class="text-xs font-semibold text-primary">{{ $session->start_at->format('H:i') }}{{ $session->end_at ? ' – '.$session->end_at->format('H:i') : '' }}</p><p class="mt-1 text-sm font-bold text-gray-900">{{ $session->schedule?->title ?? $session->class?->title ?? 'Kelas' }}</p><p class="mt-1 text-xs text-gray-500">{{ $session->studyGroup?->name ?? 'Tanpa rombel' }}</p></div>@empty<div class="flex h-full min-h-24 items-center justify-center text-center text-xs text-gray-400">Belum ada jadwal</div>@endforelse</div></article>@endforeach</div></section>
</div>
@endsection
