...
@section('content')
<div class="mx-auto max-w-3xl space-y-6" x-data="{ scope: @js(old('scope', 'personal')) }">
    ...
    <form method="POST" action="{{ route('tutor.schedule.feedback.store', $session) }}" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">@csrf
        ...
        <div x-show="scope === 'personal'"><x-ui.input.select name="user_id" label="Peserta" placeholder="Pilih peserta"><option value="" disabled>Pilih peserta</option>@foreach($students as $student)<option value="{{ $student->id }}">{{ $student->name }}</option>@endforeach</x-ui.input.select></div>
