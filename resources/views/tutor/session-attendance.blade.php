...
<div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <a href="{{ route('tutor.dashboard') }}" class="text-sm font-semibold text-primary hover:underline">← Kembali ke jadwal</a>
        ...
        <form method="POST" action="{{ route('tutor.sessions.attendance.mark', $session) }}">
            @csrf
            <button class="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/90" @disabled($session->tutorAttendance)>
                {{ $session->tutorAttendance ? 'Sudah absen: ' . ucfirst($session->tutorAttendance->status) : 'Absen Kehadiran Saya' }}
            </button>
        </form>
    </div>
            ...
            <tbody>
                @forelse($participants as $participant)
                    ...
                    <tr class="border-t border-gray-100">
                        ...
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('tutor.sessions.students.mark', $session) }}" class="flex flex-wrap items-center gap-2">
                                @csrf
