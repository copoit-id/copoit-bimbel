...
<section class="dashboard">
    <x-page-desc title="{{ __('Dashboard') }}" description="{{ __('Selamat datang') }} {{ Auth::user()->name }}"></x-page-desc>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-5 w-full">
        <div class="flex items-center gap-3 bg-white p-4 rounded-lg border border-border mt-6">
            ...
            <div>
                ...
                <p class="text-[12px] mt-[-6px] font-light">{{ __('Paket Aktif') }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 bg-white p-4 rounded-lg border border-border mt-6">
            ...
            <div>
                ...
                <p class="text-[12px] mt-[-6px] font-light">{{ __('Total Tryout') }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 bg-white p-4 rounded-lg border border-border mt-6">
            ...
            <div>
                ...
                <p class="text-[12px] mt-[-6px] font-light">{{ __('Tryout Selesai') }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 bg-white p-4 rounded-lg border border-border mt-6">
            ...
            <div>
                ...
                <p class="text-[12px] mt-[-6px] font-light">{{ __('Rata-rata Skor') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div class="lg:col-span-2 bg-white p-6 rounded-lg border border-border">
            <h3 class="text-lg font-bold mb-4">{{ __('Paket Aktif') }}</h3>
            ...
            <div class="flex items-center justify-between p-4 border border-border rounded-lg mb-3 last:mb-0">
                ...
                <div class="text-right">
                    <p class="text-sm font-medium">{{ __('Berakhir') }}: {{ $access->end_date->format('d M Y') }}</p>
            ...
            <div class="text-center py-8">
                ...
                <p class="text-gray-500">{{ __('Belum ada paket aktif') }}</p>
                <a href="{{ route('user.package.index') }}" class="text-primary hover:underline">{{ __('Lihat Paket') }}</a>
            </div>
            @endforelse
        </div>
        ...
        <div class="bg-white p-6 rounded-lg border border-border">
            <h3 class="text-lg font-bold mb-4">{{ __('Tryout Terbaru') }}</h3>
            ...
            <div class="flex items-center justify-between p-3 border-b border-border border-dashed last:border-b-0">
                ...
                <div class="text-right">
                    ...
                    <span
                        ...
                        {{ $attempt->is_passed ? __('Lulus') : __('Tidak Lulus') }}
                    </span>
                    @else
            ...
            <div class="text-center py-8">
                ...
                <p class="text-gray-500 text-sm">{{ __('Belum ada tryout') }}</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    @if($expiringSoon->count() > 0)
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-6">
            ...
            <div>
                <h4 class="font-semibold text-yellow-800">{{ __('Paket Akan Berakhir') }}</h4>
                <p class="text-yellow-700 text-sm mb-2">{{ __('Beberapa paket Anda akan berakhir dalam 7 hari:') }}</p>
                <ul class="space-y-1">
                    @foreach($expiringSoon as $access)
                    <li class="text-yellow-700 text-sm">
                        • {{ $access->package->name }} - {{ __('berakhir') }} {{ $access->end_date->format('d M Y') }}
                    </li>
                    @endforeach
                </ul>
