...
@section('title', __('Riwayat Tryout'))
@section('content')
<div class="package-bimbel bg-white p-4 rounded-lg border border-border">
    <x-page-desc title="{{ __('Riwayat') }} - {{ $tryout->name }}" description="{{ __('Lihat riwayat pengerjaan tryout Anda') }}"
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-center">{{ __('Nilai') }}</th>
                    <th scope="col" class="px-6 py-3 text-center">{{ __('Status') }}</th>
                    <th scope="col" class="px-6 py-3 text-center">{{ __('Durasi Pengerjaan') }}</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b border-dashed border-gray-200 text-grey3">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <p>{{ __('Pukul') }} {{ \Carbon\Carbon::parse($attempt['started_at'])->format('H:i') }} {{ __('WIB') }}</p>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex justify-center">
                            <a href="{{ route('user.package.tryout.pembahasan', ['id_package' => $packageId, 'id_tryout' => $tryout->tryout_id, 'token' => $attempt['attempt_token']]) }}"
                                ...
                                <span class="text-primary">{{ __('Pembahasan') }}</span>
                            </a>
                        </div>
                    </td>
                    @endif
                ...
                <tr class="bg-white border-b border-dashed border-gray-200">
                    <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <p>{{ __('Belum ada riwayat tryout') }}</p>
