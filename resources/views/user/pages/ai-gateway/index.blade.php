...
<div class="space-y-6">
    ...
    <div><h2 class="text-lg font-semibold text-gray-900">{{ $subscriptionExhausted ? 'Beli paket lagi' : 'Pilih paket' }}</h2><div class="mt-4 grid gap-4 md:grid-cols-3">@forelse($plans as $plan)@php
    ...
    @endphp<div class="flex flex-col rounded-xl border border-gray-200 bg-white p-5"><div class="flex items-start justify-between gap-3"><h3 class="font-semibold text-gray-900">{{ data_get($plan, 'name') }}</h3>@if($isFreePlan)<span class="rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-700">Gratis</span>@endif</div><p class="mt-2 text-2xl font-bold text-primary">{{ $isFreePlan ? 'Gratis' : 'Rp ' . number_format(data_get($plan, 'price'), 0, ',', '.') }}</p><p class="mt-1 text-sm text-gray-500">{{ data_get($plan, 'duration_days') > 0 ? 'Aktif ' . data_get($plan, 'duration_days') . ' hari' : 'Tanpa masa aktif' }}</p><p class="mt-4 text-sm text-gray-600">{{ data_get($plan, 'chat_limit') ? number_format(data_get($plan, 'chat_limit'), 0, ',', '.') . ' chat AI' : 'Chat AI unlimited sampai token habis' }}</p><form method="POST" action="{{ route('user.ai-gateway.checkout') }}" class="mt-5">@csrf<input type="hidden" name="plan_id" value="{{ data_get($plan, 'id') }}"><button @disabled($freePlanClaimed || $paidCheckoutBlocked) class="w-full rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">@if($freePlanClaimed)Sudah diklaim@elseif($isFreePlan)Klaim gratis@elseif($paidCheckoutBlocked)Selesaikan pembayaran sebelumnya@elseif($subscriptionExhausted)Beli lagi@else Beli paket @endif</button></form><p class="mt-3 text-center text-xs text-gray-500">{{ $isFreePlan ? 'Langsung aktif tanpa pembayaran. Satu kali klaim per akun.' : 'Pembayaran diproses oleh gateway pusat.' }}</p></div>@empty<div class="rounded-xl border border-gray-200 bg-white p-5 text-sm text-gray-500 md:col-span-3">Belum ada paket AI yang tersedia.</div>@endforelse</div></div>
