    ...
    <div class="rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            @forelse($subscriptions as $activeSubscription)
                ...
                @endphp
                <div class="rounded-xl border border-gray-100 bg-gray-50 p-4"><div class="flex items-start justify-between gap-3"><div><p class="font-semibold text-gray-900">{{ data_get($activePlan, 'name', '-') }}</p><p class="mt-1 text-xs text-gray-500">Berakhir {{ \Illuminate\Support\Carbon::parse(data_get($activeSubscription, 'ends_at'))->translatedFormat('d M Y') }}</p></div><span class="rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-green-700">Aktif</span></div><div class="mt-4">@if($activeChatLimit > 0)<div class="flex justify-between gap-3 text-xs text-gray-500"><span>Sisa chat AI</span><span>{{ number_format(max(0, $activeChatLimit - $activeChatsUsed), 0, ',', '.') }} / {{ number_format($activeChatLimit, 0, ',', '.') }}</span></div><div class="mt-2 h-2 overflow-hidden rounded-full bg-gray-200"><div class="h-full rounded-full bg-primary" style="width: {{ min(100, ($activeChatsUsed / $activeChatLimit) * 100) }}%"></div></div>@else<p class="text-xs text-gray-600">Kuota chat AI aktif.</p>@endif</div></div>
            @empty
