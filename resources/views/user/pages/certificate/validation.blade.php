...
@section('title', __('Validasi Sertifikat'))
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-lg border border-border">
        <div class="text-center">
            ...
            <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ __('Validasi Sertifikat') }}</h1>
            <p class="text-gray-600">{{ __('Verifikasi keaslian sertifikat dengan memasukkan nomor sertifikat') }}</p>
        </div>
    </div>

    <!-- Validation Form -->
    <div class="bg-white p-6 rounded-lg border border-border">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('Form Validasi') }}</h3>
        ...
        <form id="validationForm" class="space-y-4">
            <div>
                ...
                <div class="relative">
                    <input type="text" id="certificate_number" name="certificate_number"
                        ...
                        placeholder="{{ __('Contoh: 4463/CA/08/2025') }}" required>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                        <i class="ri-file-text-line text-gray-400"></i>
                    </div>
                </div>
                <p class="mt-1 text-xs text-gray-500">
                    {{ __('Masukkan nomor sertifikat sesuai format yang tertera pada sertifikat Anda') }}
                </p>
            </div>
            ...
            <button type="submit" id="validateBtn"
                ...
                <span class="btn-loading hidden">
                    <i class="ri-loader-4-line animate-spin mr-2"></i>{{ __('Memvalidasi...') }}
                </span>
            </button>
        </form>
    </div>
    ...
    <div id="validationResult" class="hidden">
        ...
        <div id="validCertificate" class="bg-white p-6 rounded-lg border border-border hidden">
            <div class="flex items-center mb-4">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-green-800">{{ __('Sertifikat Valid') }}</h3>
                    <p class="text-green-600">{{ __('Sertifikat ini asli dan terdaftar dalam sistem kami') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <h4 class="font-semibold text-gray-700 mb-3">{{ __('Informasi Sertifikat') }}</h4>
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Nama Pemegang:') }}</span>
                            <span class="font-medium" id="cert-holder"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Tanggal Lahir:') }}</span>
                            <span class="font-medium" id="cert-dob"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Program:') }}</span>
                            <span class="font-medium" id="cert-program"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Institusi:') }}</span>
                ...
                <div>
                    <h4 class="font-semibold text-gray-700 mb-3">{{ __('Detail Validitas') }}</h4>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Tanggal Kadaluarsa:') }}</span>
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('Kode Verifikasi:') }}</span>
            ...
            <!-- Scores Section -->
            <div class="mb-6">
                <h4 class="font-semibold text-gray-700 mb-3">{{ __('Hasil Ujian') }}</h4>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="text-center p-3 bg-primary/10 rounded-lg">
                        ...
                        <div class="text-xs text-primary">{{ __('Skor Keseluruhan') }}</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Certificate Preview -->
            <div class="mb-6">
                <h4 class="font-semibold text-gray-700 mb-3">{{ __('Preview Sertifikat') }}</h4>
                <div class="border-2 border-gray-200 rounded-lg p-4 bg-gray-50">
                    <img id="cert-preview" src="" alt="{{ __('Pratinjau Sertifikat') }}"
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <button onclick="resetValidation()"
                    ...
                    <i class="ri-refresh-line mr-2"></i>{{ __('Validasi Lagi') }}
                </button>
            </div>
        </div>
        ...
        <div id="invalidCertificate" class="bg-white p-6 rounded-lg border border-border hidden">
            <div class="flex items-center mb-4">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-red-800">{{ __('Sertifikat Tidak Valid') }}</h3>
                    <p class="text-red-600">{{ __('Sertifikat tidak ditemukan atau tidak terdaftar dalam sistem kami') }}</p>
                </div>
            </div>

            <div class="p-4 bg-red-50 border border-red-200 rounded-lg mb-4">
                <h4 class="font-semibold text-red-800 mb-2">{{ __('Kemungkinan Penyebab:') }}</h4>
                <ul class="text-sm text-red-700 space-y-1">
                    <li>• {{ __('Nomor sertifikat salah atau tidak sesuai format') }}</li>
                    <li>• {{ __('Sertifikat sudah kadaluarsa atau dibatalkan') }}</li>
                    <li>• {{ __('Sertifikat palsu atau tidak dikeluarkan oleh :name', ['name' => $clientBranding['name']]) }}</li>
                </ul>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <a href="#"
                    ...
                    <i class="ri-customer-service-line mr-2"></i>{{ __('Hubungi Support') }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <div class="flex items-start">
            ...
            <div>
                <h4 class="font-semibold text-blue-800 mb-2">{{ __('Informasi Validasi') }}</h4>
                <ul class="text-sm text-blue-700 space-y-1">
                    <li>• {{ __('Pastikan nomor sertifikat diketik dengan benar sesuai format') }}</li>
                    <li>• {{ __('Validasi dilakukan secara real-time terhadap database resmi') }}</li>
                    <li>• {{ __('Sertifikat yang valid akan menampilkan semua informasi lengkap') }}</li>
                    <li>• {{ __('Hubungi support jika mengalami masalah dalam validasi') }}</li>
                </ul>
            </div>
        </div>
    </div>
...
<script>
    ...
    $('#validationForm').on('submit', function(e) {
        ...
        if (!certificateNumber) {
            showAlert('error', @json(__('Nomor sertifikat wajib diisi')));
            return;
        }
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    if (errors) {
                        ...
                        showAlert('error', @json(__('Validasi gagal')));
                    }
                } else {
                    showAlert('error', @json(__('Terjadi kesalahan saat validasi')));
                }
            }
        });
    }
    ...
    function showValidCertificate(data) {
        ...
        $('#cert-number').text(data.certificate_number || @json(__('Unknown')));
        $('#cert-holder').text(data.holder_name || @json(__('Unknown')));
        $('#cert-dob').text(data.date_of_birth || @json(__('Unknown')));
        $('#cert-program').text(data.package_name || @json(__('Unknown')));
        $('#cert-institution').text(data.institution_name || @json(__('Unknown')));
        $('#cert-issued').text(data.issued_date || @json(__('Unknown')));
        $('#cert-expired').text(data.expired_date || @json(__('Unknown')));
        $('#cert-verification').text(data.verification_code || @json(__('Unknown')));

        // Set status with color
        const statusEl = $('#cert-status');
        statusEl.removeClass('text-red-600 text-green-600'); // Clear previous classes
        if (data.is_expired) {
            statusEl.text(@json(__('Kadaluarsa'))).addClass('text-red-600');
        } else {
            statusEl.text(@json(__('Aktif'))).addClass('text-green-600');
        }
    ...
    $('#downloadCertBtn').on('click', function() {
        if (!currentCertificateData || !currentCertificateData.certificate_id) {
            showAlert('error', @json(__('Data sertifikat tidak ditemukan')));
            return;
        }
        ...
        try {
            ...
            showAlert('error', @json(__('Gagal mengunduh sertifikat')));
        }
    });
