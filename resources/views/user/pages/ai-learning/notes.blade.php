...
@section('title', 'Catatan Dipin')

@section('content')
<div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><p class="text-sm text-gray-500">AI Learning Tools</p><h1 class="text-2xl font-semibold text-gray-900">Catatan Dipin</h1><p class="mt-1 text-sm text-gray-500">Ringkasan materi yang kamu pin dari pembahasan soal.</p></div>
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        @forelse($notes as $note)
            <article class="flex flex-col rounded-xl border border-gray-200 bg-white p-5">
                ...
                <p class="mt-3 text-xs text-gray-400">Dipin {{ $note->saved_at?->translatedFormat('d M Y H:i') }}</p>
