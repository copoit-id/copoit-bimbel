...
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Paket {{ $scopes[$activeScope] }}</h2>
            <p class="text-gray-500">{{ $activeScope === 'admin_question_generator' ? 'Kelola kuota berbayar khusus admin untuk membuat soal AI.' : 'Kelola paket AI berbayar atau gratis untuk Pembahasan AI dan AI Learning Tools peserta.' }}</p>
        </div>
    ...
    <nav class="flex w-fit rounded-xl border border-gray-200 bg-white p-1" aria-label="Kategori paket AI">
        @foreach($scopes as $scope => $label)
            <a href="{{ route('super-admin.ai-gateway-plans.index', ['scope' => $scope]) }}" class="rounded-lg px-4 py-2 text-sm font-semibold {{ $activeScope === $scope ? 'bg-primary text-white' : 'text-gray-600 hover:bg-gray-50' }}">{{ $label }}</a>
        @endforeach
    </nav>
        ...
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Isi harga 0 untuk paket gratis yang langsung aktif ketika diklaim.</p>
                </div>
