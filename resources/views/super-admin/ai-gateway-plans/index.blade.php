...
<div class="space-y-6">
    ...
    <div class="rounded-xl border border-border bg-white p-6">
        <div class="mb-4">
            ...
            <p class="mt-1 text-sm text-gray-500">Setiap paket wajib memiliki batas token. Isi limit chat 0 agar jumlah chat unlimited sampai token habis.</p>
        </div>
                ...
                <div class="rounded-xl border border-gray-200 p-5">
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">{{ $plan->slug }} · Rp {{ number_format($plan->price, 0, ',', '.') }} / {{ $plan->duration_days > 0 ? $plan->duration_days . ' hari' : 'tanpa masa aktif' }}</p>
                        </div>
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-4 border-t border-gray-100 pt-4 text-sm md:grid-cols-3">
                        ...
                        <div>
                            ...
                            <span class="ml-1 font-medium text-gray-800">{{ $plan->token_limit ? number_format($plan->token_limit, 0, ',', '.') : 'Perlu diatur' }}</span>
                        </div>
                        <div>
    ...
    <div class="flex min-h-full items-center justify-center">
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form method="POST" action="{{ route('super-admin.ai-gateway-plans.store') }}" class="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <label class="block"><span class="text-sm font-semibold text-gray-700">Masa Aktif (hari)</span><input name="duration_days" type="number" min="0" value="30" required class="mt-1.5 w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10"><span class="mt-1 block text-xs text-gray-500">Isi 0 untuk tanpa masa aktif.</span></label>
                ...
                <label class="block"><span class="text-sm font-semibold text-gray-700">Limit Chat</span><input name="chat_limit" type="number" min="0" value="0" required class="mt-1.5 w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10"><span class="mt-1 block text-xs text-gray-500">Isi 0 untuk unlimited; penggunaan berhenti saat token habis.</span></label>
