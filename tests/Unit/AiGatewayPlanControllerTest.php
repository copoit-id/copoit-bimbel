...
class AiGatewayPlanControllerTest extends TestCase
{
    ...
    public function test_question_generator_plan_is_always_lifetime(): void
    {
        $request = Request::create('/super-admin/ai-gateway-plans', 'POST', [
            'scope' => AiGatewayPlan::SCOPE_ADMIN_QUESTION_GENERATOR,
            'name' => 'Paket Generate Soal',
            'price' => 25000,
            'token_limit' => 50000,
            'chat_limit' => 0,
            'duration_days' => 30,
        ]);
        $controller = app(AiGatewayPlanController::class);
        $validated = (new ReflectionMethod(AiGatewayPlanController::class, 'validated'))->invoke($controller, $request);

        $normalized = (new ReflectionMethod(AiGatewayPlanController::class, 'normalizeScopeDefaults'))
            ->invoke($controller, $validated);

        $this->assertSame(0, $normalized['duration_days']);
    }
