...
class TutorContentVisibilityTest extends TestCase
{
    ...
    public function test_isolated_mode_limits_tutor_content_to_its_owner(): void
    {
        ...
        $this->assertSame(['Tryout Tutor Satu'], Tryout::query()->pluck('name')->all());
        $this->assertSame(['Materi Tutor Satu'], Material::query()->pluck('title')->all());
        $this->assertSame(['Bank Tutor Satu'], QuestionBank::query()->pluck('name')->all());
        $this->assertSame([1], TryoutDetail::query()->pluck('tryout_detail_id')->all());
        $this->assertSame([1], Question::query()->pluck('question_id')->all());
        $this->assertSame([1], QuestionBankQuestion::query()->pluck('id')->all());

        $this->actingAs(User::findOrFail(103));

        $this->assertCount(2, Tryout::query()->get());
        $this->assertCount(2, Material::query()->get());
        $this->assertCount(2, QuestionBank::query()->get());
        $this->assertCount(2, TryoutDetail::query()->get());
        $this->assertCount(2, Question::query()->get());
