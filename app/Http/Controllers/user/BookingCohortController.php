...
class BookingCohortController extends Controller
{
    public function store(Request $request, GroupBookingService $service): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('success', "Kelompok dibuat. Bagikan kode {$cohort->invite_code} kepada anggota.");
    }
    ...
    public function join(Request $request, GroupBookingService $service): RedirectResponse
    {
        ...
        $cohort = $service->join($request->user(), $validated['invite_code']);

        return redirect()
            ->route('user.booking.index')
            ->with('success', "Berhasil bergabung ke kelompok {$cohort->invite_code}. Tagihanmu sudah dibuat.");
    }
}
