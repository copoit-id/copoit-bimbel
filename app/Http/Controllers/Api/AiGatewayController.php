...
use App\Http\Controllers\Controller; use App\Models\AiGatewayClient; use App\Models\AiGatewayUsageLog; use App\Services\AiDiscussionService; use Illuminate\Http\Request; use Illuminate\Support\Str;
class AiGatewayController extends Controller {
 public function discussion(Request $request, AiDiscussionService $ai) {
  $key=(string)$request->header('X-AI-Gateway-Key'); $client=AiGatewayClient::where('api_key_hash',hash('sha256',$key))->where('is_active',true)->first();
  ...
  if($client->monthly_token_limit>0 && $used >= $client->monthly_token_limit) return response()->json(['message'=>'Kuota token project ini habis.'],429);
  try { $result=$ai->chat($data['message'],$data['context'], true); } catch(\RuntimeException $e) { return response()->json(['message'=>$e->getMessage()],422); }
  ...
  $client->update(['last_used_at'=>now()]); return response()->json($result);
 }
}
