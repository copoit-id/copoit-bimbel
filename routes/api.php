...
Route::post('/webhook/ai-gateway/xendit', [\App\Http\Controllers\Api\AiGatewayBillingController::class, 'webhook'])->middleware('throttle:120,1')->name('webhook.ai-gateway.xendit');
Route::post('/webhook/ai-gateway/midtrans', [\App\Http\Controllers\Api\AiGatewayBillingController::class, 'midtransWebhook'])->middleware('throttle:120,1')->name('webhook.ai-gateway.midtrans');
Route::post('/webhook/ai-gateway/ipaymu', [\App\Http\Controllers\Api\AiGatewayBillingController::class, 'ipaymuWebhook'])->middleware('throttle:120,1')->name('webhook.ai-gateway.ipaymu');
Route::get('/ai-gateway/payments/{externalId}/qris/status', [\App\Http\Controllers\Api\AiGatewayBillingController::class, 'qrisStatus'])->middleware('throttle:60,1')->name('ai-gateway-payments.qris.status');
