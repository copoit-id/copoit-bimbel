...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $now = now();
        $label = (string) data_get(config('permissions.features', []), 'attendance.label', 'Absensi Kelas');
        ...
        foreach (array_keys(config('permissions.actions', [])) as $action) {
            ...
            $attendancePermissionId = DB::table('permissions')
                ->where('slug', 'attendance.'.$action)
                ->value('id');

            if (! $attendancePermissionId || $roleIds->isEmpty()) {
                continue;
            }

            DB::table('permission_role')->insertOrIgnore(
                $roleIds->map(fn ($roleId): array => [
                    'permission_id' => $attendancePermissionId,
                    'role_id' => $roleId,
                ])->all()
            );
        }
